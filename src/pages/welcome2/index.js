import React from "react";
import { View, Text, StatusBar, Image, StyleSheet, TouchableOpacity } from 'react-native';
import Colours from '../../../assets/colours';
import { useNavigation } from "@react-navigation/native";

export default function Welcome2() {
  const navigation = useNavigation();

  return (
    <View style={styles.container}>
      
      <StatusBar hidden={true} />

      <View style={styles.pictureContainer}>
        <Image
          source={require('../../../assets/images/tela2.png')}
          style={styles.image}
          resizeMode="contain"
        />
      </View>
      
      <View style={styles.board}>
        <Text style={styles.text}>
        Não sabe onde divulgar seus projetos e obter apoio financeiro para os mesmos?
        </Text>

        <TouchableOpacity 
        style={styles.skipButton}
        onPress ={ ()=> navigation.navigate('Signin')}
        >
          <Text style={styles.textButton}>pular</Text>
        </TouchableOpacity>

        <TouchableOpacity 
        style={styles.nextButton}
        onPress={() => navigation.navigate('Welcome3')}>
        
          <Text style={styles.textButton}>próximo</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
}

const styles = StyleSheet.create({ 
  container: { 
    flex: 1,
    backgroundColor: Colours.backgroundColour
  },

  pictureContainer: {
    flex: 2,
    width: '100%',
    justifyContent: 'center',
    alignItems: 'center',
    paddingBottom: 0,
    zIndex: 1 // Garante que a imagem fique na frente da view board
  },

  image: {
    width: '60%',
    position: 'absolute', // Faz a imagem flutuar
    top: -650, // Ajuste conforme necessário para sobrepor a view
    zIndex: 2 // Certifica que a imagem está acima de todas as views
  },

  board: {
    flex: 2,
    backgroundColor: Colours.headerColour,
    justifyContent: 'center',
    alignItems: 'center',
    borderTopRightRadius: 25,
    borderTopLeftRadius: 25,
    position: 'relative',
    zIndex: 0 // Garante que a view board fique atrás da imagem
  },

  text: {
    fontSize: 20,
    width: '60%',
    textAlign: 'center',
    color: Colours.offWhite,
    fontWeight: 'bold'
  },

  skipButton:{
    position: 'absolute',
    left: '8%',
    bottom: '10%',
    zIndex: 2
  },

  nextButton: {
    position: 'absolute',
    right: '8%',
    bottom: '10%',
    zIndex: 2
  },

  textButton:{
    color: Colours.offWhite
  }

});
